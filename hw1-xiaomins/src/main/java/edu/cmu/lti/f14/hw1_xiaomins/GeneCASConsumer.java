...
/**
 * @author Ryan Sun The CAS consumer receives each CAS after it has been analyzed by the Analysis
...
public class GeneCASConsumer extends CasConsumer_ImplBase {
  ...
  public void processCas(CAS aCAS) throws ResourceProcessException {
    ...
    FSIterator<Annotation> it = jcas.getAnnotationIndex(type.type).iterator();
    while(it.hasNext()){
      ...
      start -= countSpace(thisType.getSentence(), start);
      end -= countSpace(thisType.getSentence(), end) + 1;
